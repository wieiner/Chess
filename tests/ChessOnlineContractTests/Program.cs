...
static void OnlineClientSdkTests(ContractTest test)
{
    ...
    test.Check(ChessOnlineRelayEvents.All.Contains("ReceiveLobbySnapshot"), "online relay client registers lobby snapshot callback event");
    ...
    test.Check(relayClient.ReconnectState.State == OnlineConnectionState.Disconnected &&
        ...
        relayClient.LastLobbySnapshot == null &&
    ...
    var lobbyFilter = new OnlineLobbyFilterState
    {
        RulesetIdFilter = " classic-six-side-3d-8x8x8-v0.1 ",
        IncludeWaitingTables = false,
        IncludeInGameTables = true,
        IncludeFinishedTables = false
    };
    var lobbyRequest = lobbyFilter.ToRequest();
    test.Check(lobbyRequest.RulesetIdFilter == "classic-six-side-3d-8x8x8-v0.1" &&
        !lobbyRequest.IncludeWaitingTables &&
        lobbyRequest.IncludeInGameTables &&
        !lobbyRequest.IncludeFinishedTables,
        "online lobby filter builds trimmed request");

    var lobbyRows = OnlineLobbyTableDisplayRow.FromSnapshot(new OnlineLobbySnapshot
    {
        Tables =
        {
            new OnlineLobbyTableRow
    ...
    test.Check(lobbyRows.Count == 1 &&
        lobbyRows[0].CanSpectate &&
        lobbyRows[0].CanJoinAsPlayer &&
        lobbyRows[0].SeatSummary.Contains("ready", StringComparison.OrdinalIgnoreCase) &&
        !lobbyRows[0].DisplayLabel.Contains("accessToken", StringComparison.OrdinalIgnoreCase),
        "online lobby display row handles active table without secrets");
